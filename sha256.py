# original source: https://bitbucket.org/pypy/pypy/src/tip/lib_pypy/_sha256.py
...
ROR = lambda x, y: (((x & 0xffffffff) >> (y & 31)) | (
...
def sha_transform(sha_info):
    ...
    for i in range(0, 16):
        W.append(
            (d[4 * i] << 24) + (d[4 * i + 1] << 16) + (d[4 * i + 2] << 8) + d[
                4 * i + 3])

    for i in range(16, 64):
        W.append((Gamma1(W[i - 2]) + W[i - 7] + Gamma0(W[i - 15]) + W[
            i - 16]) & 0xffffffff)
...
def sha_final(sha_info):
    ...
    if count > SHA_BLOCKSIZE - 8:
        ...
        sha_info['data'] = sha_info['data'][:count] + bytearray(
                    [0] * (SHA_BLOCKSIZE - count))
        sha_transform(sha_info)
        # zero bytes in data
        sha_info['data'] = [0] * SHA_BLOCKSIZE
    else:
        sha_info['data'] = sha_info['data'][:count] + (
                    [0] * (SHA_BLOCKSIZE - count))
...
class sha256:
    ...
    def digest(self):
        return bytes(sha_final(self._sha.copy())[:self._sha['digestsize']])
...
def test():
    ...
    assert 'd7b553c6f09ac85d142415f857c5310f3bbbe7cdd787cce4b985acedd585266f' == sha256(
        a_str).hexdigest()
    assert '8113ebf33c97daa9998762aacafe750c7cefc2b2f173c90c59663a57fe626f21' == sha256(
        a_str * 7).hexdigest()
